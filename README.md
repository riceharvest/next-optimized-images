# :sunrise: @opensourceframework/next-optimized-images [![npm version](https://badgen.net/npm/v/@opensourceframework/next-optimized-images)](https://www.npmjs.com/package/@opensourceframework/next-optimized-images) [![license](https://badgen.net/github/license/cyrilwanner/@opensourceframework/next-optimized-images)](https://github.com/cyrilwanner/@opensourceframework/next-optimized-images/blob/master/LICENSE) [![downloads](https://badgen.net/npm/dt/@opensourceframework/next-optimized-images)](https://www.npmjs.com/package/@opensourceframework/next-optimized-images)

**:bulb: Version 3 is coming!**
It introduces a complete rewrite with many new features and bugfixes. If you want to help developing and testing the upcoming major version, please check out the [canary branch](https://github.com/cyrilwanner/@opensourceframework/next-optimized-images/tree/canary) for installation instructions and more information about the new features. ([RFC issue](https://github.com/cyrilwanner/@opensourceframework/next-optimized-images/issues/120))

---

Automatically optimize images used in [next.js](https://github.com/zeit/next.js) projects (`jpeg`, `png`, `svg`, `webp` and `gif`).

Image sizes can often get reduced between 20-60%, but this is not the only thing `@opensourceframework/next-optimized-images` does:
...
- **Original Repository**: https://github.com/cyrilwanner/next-optimized-images
...
*Node >= 8 is required for version 2. If you need to support older node versions, you can still use [version 1](https://github.com/cyrilwanner/@opensourceframework/next-optimized-images/tree/v1#readme) of @opensourceframework/next-optimized-images.*
...
module.exports = withPlugins([
  [optimizedImages, {
    /* config for @opensourceframework/next-optimized-images */
  }],
...
The example above uses [next-compose-plugins](https://github.com/cyrilwanner/next-compose-plugins) for a cleaner API when using many plugins, see its readme for a more detailed example. `@opensourceframework/next-optimized-images` also works with the standard plugin api:
...
Starting from version 2, you have to install the optimization packages you need in your project in addition to this plugin. `@opensourceframework/next-optimized-images` then detects all the supported packages and uses them.
...
Otherwise, `@opensourceframework/next-optimized-images` will copy your image into the static folder of next.js and the `require(...)` returns the path to your image in this case (`/_next/static/images/my-image-5216de428a8e8bd01a4aa3673d2d1391.jpg`).

You can use both variants directly on an image in the `src` attribute or in your CSS file inside an `url()` value.

### Query params

> *If you are using flow or eslint-plugin-import and are experiencing some issues with query params, check out the [solution posted by @eleith](https://github.com/cyrilwanner/@opensourceframework/next-optimized-images/issues/23).*
...
If this `?webp` query parameter is specified, `@opensourceframework/next-optimized-images` automatically converts a JPEG/PNG image to the new WebP format.
...
`@opensourceframework/next-optimized-images` registers the webpack loader for all these file types.
If you don't want one of these handled by @opensourceframework/next-optimized-images because you, for example, have another plugin or custom loader rule, simply remove it from the array.
...
From version 2 on, @opensourceframework/next-optimized-images uses the [`assetPrefx` config of next.js](https://nextjs.org/docs/#cdn-support-with-asset-prefix) by default, but you can overwrite it with `imagesPublicPath` specially for images.
...
```javascript
{
  runtimeGenerator: require.resolve(path.resolve('node_modules', '@opensourceframework/next-optimized-images', 'svg-runtime-generator.js')),
}
```
...
Just be aware that you can't use any of the query params `@opensourceframework/next-optimized-images` provides anymore on these images because the request just gets forwarded and not modified anymore.
...
[MIT](https://github.com/cyrilwanner/@opensourceframework/next-optimized-images/blob/master/LICENSE) © Cyril Wanner
